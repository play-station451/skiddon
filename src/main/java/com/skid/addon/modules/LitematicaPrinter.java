...
import com.skid.addon.utils.PrinterUtils;
...
public class LitematicaPrinter extends Module {
	...
	public LitematicaPrinter() {
		super(Skiddon.CATEGORY, "litematica-printer", "Automatically prints open schematics");
	}
		...
		if (timer >= printing_delay.get()) {
			BlockIterator.register(printing_range.get() + 1, printing_range.get() + 1, (pos, blockState) -> {
				...
				if (
					...
					boolean isBlockInLineOfSight = PrinterUtils.isBlockInLineOfSight(pos, required);
					...
					if(
						...
						&& PrinterUtils.getVisiblePlaceSide(
							pos,
							required,
							wantedSlabType, 
							wantedBlockHalf,
	...
	public boolean place(BlockState required, BlockPos pos) {
        ...
        return PrinterUtils.place(pos, placeSide, wantedSlabType, wantedBlockHalf, wantedHorizontalOrientation != null ? wantedHorizontalOrientation : wantedHopperOrientation, wantedAxies, airPlace.get(), swing.get(), rotate.get(), clientSide.get(), printing_range.get());
	}
